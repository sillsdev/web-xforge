...
public class Migrator : DisposableBase
{
            ...
            foreach (string sfProjectId in sfProjectIds)
            {
                ...
                if (projectIsSyncing)
                {
                    ...
                    if (projectIsSyncing)
                    {
                        ...
                        if (writeMode)
                        {
                            // The SF-1444 fix will soon be applied, and this project should not be synced until it can
                            // be properly migrated.
                            _projectStats.Rows.Find(project.Id)!["SyncShouldStayDisabled"] = true;
                        }
                        else
                        {
                            await SetSyncDisabledUnlessBeganDisabled(sfProjectDoc, syncBeganDisabled, false);
                        }
    ...
    private async Task SetSyncDisabledUnlessBeganDisabled(
    ...
    )
    {
        if (syncBeganDisabled)
            return;
        await SetSyncDisabled(sfProjectDoc, syncDisabled);
        if (!syncDisabled)
        {
