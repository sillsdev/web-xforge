...
export function issuesEmailTemplate(errorId?: string): string {
  ...
  const template = `mailto:${environment.issueEmail}?subject=${environment.siteName} issue&body=`;
...
Error id: ${errorId || 'not applicable'}`;
  // Encode all reserved characters in the body because some chars cause problems (i.e. truncating the body)
  return encodeURI(template) + encodeURIComponent(body);
}
