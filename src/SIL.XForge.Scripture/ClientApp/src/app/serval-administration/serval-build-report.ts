...
export function interpretTypes(report: ServalBuildReportDto): ServalBuildReportDto {
  ...
  const status: string = report.status as string;
  if (!isDraftGenerationBuildState(status)) {
    throw new Error(`Unknown DraftGenerationBuildState status: ${status}`);
  }
 ...
 * Values match the C# DraftGenerationBuildState enum.
 */
export const DraftGenerationBuildState = {
...
export type DraftGenerationBuildState = (typeof DraftGenerationBuildState)[keyof typeof DraftGenerationBuildState];

/** Set of all valid DraftGenerationBuildState values. */
const draftGenerationBuildStateValues: Set<string> = new Set(Object.values(DraftGenerationBuildState));

/** Returns whether the given string is a valid DraftGenerationBuildState value. */
export function isDraftGenerationBuildState(value: string): value is DraftGenerationBuildState {
  return draftGenerationBuildStateValues.has(value);
}
