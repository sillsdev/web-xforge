...
fdescribe('TrainingDataUploadDialogComponent', () => {
  ...
  it('should upload training data and return the object on save', async () => {
    const promiseForResult: Promise<TrainingDataUploadDialogResult> = env.dialogRef.afterClosed().toPromise();
    env.component.updateTrainingData(env.trainingDataFile);
    await env.component.save();
    await env.wait();

    const result: TrainingDataUploadDialogResult = await promiseForResult;

    expect(result.dataId).not.toEqual('');
    expect(result.dataId.length).toEqual(20); // object id length
  });
