...
export class JobDetailsDialogComponent implements OnInit {
  ...
  get languagePairTooltip(): string {
    if (this.data.buildsStartedSince === 1) {
      return (
        'This language pair comes from the current engine configuration. ' +
        '1 build has started since this build, so the language pair may have changed and may not reflect what was used for this build.'
      );
    } else {
      return (
        'This language pair comes from the current engine configuration. ' +
        `${this.data.buildsStartedSince} builds have started since this build, so the language pair may have changed and may not reflect what was used for this build.`
      );
    }
  }
