...
export class LynxService {
  ...
  private async onProjectActivated(projectDoc: SFProjectProfileDoc): Promise<void> {
  ...
  private async onBookChapterActivated(bookChapter: RouteBookChapter): Promise<void> {
    const textDocId =
    ...
    if (this.textDocId != null) {
      ...
      this.textDocChangeSubscription = textDoc.changes$
        ...
        .subscribe(async changes => {
          await this.documentManager.fireChanged(textDocId.toString(), {
...
function getTextDocIds(projectDoc: SFProjectProfileDoc): Set<string> {
  if (projectDoc == null) {
    return new Set();
  }
  return new Set(
