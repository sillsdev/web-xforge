...
{
    public class MemoryRepository : IRepository<T> where T : IIdentifiable
    {
        ...
        public MemoryRepository(IEnumerable<Func<T, object>> uniqueKeySelectors = null, IEnumerable<T> entities = null)
        {
            ...
            for (int i = 0; i < _uniqueKeys.Length; i++) _uniqueKeys[i] = new HashSet<object>();

            _entities = new ConcurrentDictionary<string, string>();
            if (entities != null) Add(entities);
        }
        ...
        public void Add(T entity)
        {
            for (int i = 0; i < _uniqueKeySelectors.Length; i++)
            {
                ...
                if (key != null) _uniqueKeys[i].Add(key);
            }
        ...
        public void Remove(T entity)
        {
            ...
            {
                object key = _uniqueKeySelectors[i](entity);
                if (key != null) _uniqueKeys[i].Remove(key);
            }
        ...
        public Task InsertAsync(T entity)
        {
            ...
            if (_entities.ContainsKey(entity.Id) || CheckDuplicateKeys(entity)) throw new DuplicateKeyException();
        ...
        public Task<T>
        UpdateAsync(Expression<Func<T, bool>> filter, Action<IUpdateBuilder<T>> update, bool upsert = false)
        {
            ...
            if (entity != null || upsert)
            {
                ...
                if (isInsert)
                {
                    ...
                    if (binaryExpr != null)
                    {
                        ...
                        if (value is string) id = (string) value;
                    }
                    entity.Id = id;
                }
                else
                ...
                if (CheckDuplicateKeys(entity, original)) throw new DuplicateKeyException();
        ...
        public Task<int> DeleteAllAsync(Expression<Func<T, bool>> filter)
        {
            ...
            foreach (T entity in entities) Remove(entity);
        ...
        private bool CheckDuplicateKeys(T entity, T original = default(T))
        {
            for (int i = 0; i < _uniqueKeySelectors.Length; i++)
            {
                ...
                if (key != null)
                {
                    ...
                    {
                        if (original == null || !key.Equals(_uniqueKeySelectors[i](original))) return true;
                    }
                }
            }
            return false;
