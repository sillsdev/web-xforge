...
export class ServalBuildsComponent extends DataLoadingComponent implements OnInit {
  ...
  protected statusIcon(status: DraftGenerationBuildState): string {
    switch (status) {
      case DraftGenerationBuildState.Completed:
        return 'done';
      case DraftGenerationBuildState.Active:
      case DraftGenerationBuildState.Finishing:
        return 'miscellaneous_services';
      case DraftGenerationBuildState.UserRequested:
      case DraftGenerationBuildState.SubmittedToServal:
      case DraftGenerationBuildState.Queued:
      case DraftGenerationBuildState.Pending:
        return 'hourglass_top';
      case DraftGenerationBuildState.Canceled:
        return 'cancel';
      case DraftGenerationBuildState.Faulted:
        return 'error';
      default:
        return 'help';
    }
  ...
  protected static formatStatusLabel(status: DraftGenerationBuildState): string {
    if (status === DraftGenerationBuildState.UserRequested) {
      return 'User requested';
    }
    if (status === DraftGenerationBuildState.SubmittedToServal) {
  ...
  private buildSummaryDisplayItems(summary: ServalBuildSummary | undefined): SummaryDisplayItem[] {
    ...
    const inProgressItem: SummaryDisplayItem = {
      ...
      explanation: 'Builds in state UserRequested, SubmittedToServal, Queued, Pending, Active, or Finishing.',
