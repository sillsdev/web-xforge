...
public class ParatextControllerTests
{
    ...
    public async Task CreateProjectAsync_Forbidden()
    {
        ...
        env.ProjectService.CreateProjectAsync(User01, Arg.Is<SFProjectCreateSettings>(s => s.ParatextId == Paratext01))
            .ThrowsAsync(new ForbiddenException("Forbidden"));

        // SUT
        ActionResult<string> actual = await env.Controller.CreateProjectAsync(Paratext01);
    ...
    public async Task CreateProjectAsync_Success()
    {
        ...
        var env = new TestEnvironment();
        env.ProjectService.CreateProjectAsync(User01, Arg.Is<SFProjectCreateSettings>(s => s.ParatextId == Paratext01))
            .Returns(Task.FromResult(Project01));
