    ...
    public class PTDDSyncRunner : IPTDDSyncRunner
    {
        ...
        public async Task RunAsync(string projectId, string userId, bool trainEngine, bool silent, bool pushLocal)
        {
            try
            {
                ...
                {
                    permissions = await _paratextService.GetPermissionsAsync(
                        _userSecret,
                        _projectDoc.Data,
                        ptUsernameMapping
                    );
                ...
                foreach (int bookNum in targetBooks)
                {
                    ...
                    if (
                        questionDocsByBook.TryGetValue(
                            text.BookNum,
                    ...
                    await _projectDoc.SubmitJson0OpAsync(op =>
                    {
                        ...
                        else
                        {
                            ...
                            op.Set(
                                pd => pd.Texts[textIndex].Permissions,
                                permissions,
        ...
        private async Task CompleteSync(bool successful)
        {
            ...
            else
            {
                try
                {
                    ptUserRoles = await _paratextService.GetProjectRolesAsync(_userSecret, _projectDoc.Data.ParatextId);
                }
            ...
            await _projectDoc.SubmitJson0OpAsync(op =>
            {
                ...
                {
                    bool sourceIsRtl = _paratextService.IsProjectLanguageRightToLeft(
                        _userSecret,
