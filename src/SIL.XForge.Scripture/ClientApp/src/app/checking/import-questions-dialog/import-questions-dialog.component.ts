...
export class ImportQuestionsDialogComponent extends SubscriptionDisposable {
      ...
      const alreadyImportedQuestion =
        questionQuery.docs.find(
          doc =>
            ...
            new VerseRef(question.book, question.startChapter, question.startVerse).BBBCCCVVV ===
              new VerseRef(doc.data.verseRef.bookNum, doc.data.verseRef.chapterNum, doc.data.verseRef.verseNum)
                .BBBCCCVVV
        ) || null;
  ...
  reference(q: TransceleratorQuestion): string {
  ...
  isBetweenRefs(ref: VerseRef, from: VerseRef | null, to?: VerseRef | null): boolean {
  ...
  async confirmEditsIfNecessary(changes: DialogListItem[]): Promise<void> {
    ...
    Promise.all(
      listItems.map(listItem => {
        ...
        if (listItem.alreadyImportedQuestion) {
          if (
            listItem.alreadyImportedQuestion.data &&
            listItem.alreadyImportedQuestion.data.text !== listItem.question.text
          ) {
            listItem.alreadyImportedQuestion.submitJson0Op(op =>
              op.set(q => q.text!, listItem.question.text).set(q => q.dateModified, currentDate)
            );
          }
        } else {
          const verse = new VerseRef(
            listItem.question.book,
            listItem.question.startChapter,
            listItem.question.startVerse
          );
