...
public class Migrator : DisposableBase
{
        ...
        try
        {
            ...
            foreach (SFProject project in projects)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Program.Log(
                        $"sf proj {project.Id}: Error: Unhandled error while processing project. Skipping. Exception thrown: {e}"
                    );
                    allSuccess = false;
                    continue;
                }
    ...
    private void ReportStats()
    {
        ...
        Program.Log($"Stats: Projects with Error: {_projectStats.Select("Error = true").Length}");
