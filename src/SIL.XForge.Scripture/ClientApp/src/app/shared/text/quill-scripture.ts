...
export function registerScripture(): void {
  ...
  class FixSelectionHistory extends QuillHistory {
    /**
     * Performs undo/redo. Override this method, so that we can fix the selection logic.
     *
    ...
    change(source: HistoryStackType, dest: HistoryStackType): void {
      ...
      const index = this.getLastChangeIndex(delta[source]);
      this.quill.setSelection(index);
    }

    endsWithNewlineChange(delta: DeltaStatic): boolean {
      if (delta.ops == null) {
        return false;
      }
    ...
    getLastChangeIndex(delta: DeltaStatic): number {
      if (delta.ops == null) {
        return 0;
      }
      ...
      if (this.endsWithNewlineChange(delta)) {
        changeIndex -= 1;
      }
      return changeIndex;
    }
