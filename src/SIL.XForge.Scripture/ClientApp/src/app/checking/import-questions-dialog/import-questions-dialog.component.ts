  ...
  async fileSelected(file: File) {
    ...
    for (const [index, row] of result.entries()) {
      ...
      try {
        ...
        // the reference
        const refStartsWithBook = Canon.allBookIds.includes(reference.slice(0, 3)) && reference[3] === ' ';
        const fullReference = refStartsWithBook || defaultBookId == null ? reference : defaultBookId + ' ' + reference;
        questions.push({
