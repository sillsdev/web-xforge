...
async function selectSource(
  ...
  const inputIndex = await indexForInput(canvasElement, fieldName);
...
async function indexForInput(
  canvasElement: HTMLElement,
  fieldName: 'training_a' | 'training_b' | 'drafting'
): Promise<number> {
  const secondTrainingFieldShown =
    (await within(canvasElement).queryByRole('button', { name: /Add another reference project/ })) == null;
  if (!secondTrainingFieldShown && fieldName === 'training_b') throw new Error('Second training field is not visible');
