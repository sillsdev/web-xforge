...
namespace Migrations
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            try
            {
                List<string> projectPaths = new List<string>(Directory.EnumerateDirectories(syncDir));
        ...
        private static async Task ProcessUsxFilesAsync(string path, bool doWrite)
        {
            ...
            var oldFiles = from file in Directory.EnumerateFiles(path, "*.xml", SearchOption.TopDirectoryOnly)
                           let line3 = File.ReadLines(file).ElementAtOrDefault(2) ?? ""
                           where line3.StartsWith(" ")
                           select file;
            int usxFileCount = Directory.EnumerateFiles(path, "*.xml", SearchOption.TopDirectoryOnly).Count();
            Console.WriteLine($"{oldFiles.Count()}/{usxFileCount} old/total files found.");
