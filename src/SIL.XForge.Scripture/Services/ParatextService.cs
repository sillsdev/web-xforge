...
namespace SIL.XForge.Scripture.Services
{
    ...
    public class ParatextService : DisposableBase, IParatextService
    {
        ...
        /// <summary>Returns the currently revision of the hg repo that a user accesses for a project PT id.</summary>
        public string GetRepoRevision(UserSecret userSecret, string projectPTId)
        {
            if (IsResource(projectPTId))
            {
                throw new InvalidOperationException("Can not query a resource for an hg repo revision.");
            }
            using ScrText scrText = GetScrText(userSecret, projectPTId);
            return _hgHelper.GetRepoRevision(scrText.Directory);
        }

        /// <summary>For the project hg repo accessed by a user, set the repo to a Mercurial revision.</summary>>
        public void SetRepoToRevision(UserSecret userSecret, string projectPTId, string desiredRevision)
        {
            ...
            {
                throw new Exception(
                    $"SetRepoToRevision failed to set repo for project PT id {projectPTId} to revision {desiredRevision}, as resulting revision is {currentRepoRev}."
                );
            }
        }
