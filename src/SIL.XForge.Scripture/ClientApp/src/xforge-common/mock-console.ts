 ...
 * Or as:
 * ```
 * mockedConsole.shouldOnlyGet(/record not found/);
 * ...SUT...
...
export class MockConsole extends AbstractFunctionLogger {
  ...
  assertHasOnlyLog(message: RegExp, moreContext: string = '') {
    ...
    if (!has) {
      ...
      fail(`should have contained expected log item. ${moreContext}`);
    } else {
      if (logCount > 1) {
        this.originalConsole.log('The following was logged:');
        this.logs.forEach(this.originalConsole.log);
        fail(`item was logged but not alone. ${moreContext}`);
      }
    }
  }

  /** Specify a log message to mute and later check for when verifying. */
  shouldGet(message: RegExp, context: string = ''): void {
  ...
  shouldOnlyGet(message: RegExp, context: string = ''): void {
