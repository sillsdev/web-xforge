...
const mockedGenerateDraftAuthGuard = mock(GenerateDraftAuthGuard);
...
describe('AppComponent', () => {
  configureTestingModule(() => ({
    ...
    providers: [
      ...
      { provide: GenerateDraftAuthGuard, useMock: mockedGenerateDraftAuthGuard },
  ...
  it('hides generate draft when user does not have access', fakeAsync(() => {
    ...
    expect(env.isDrawerVisible).toEqual(true);
    expect(env.component.isTranslateEnabled).toEqual(true);
    // Translate | Community Checking | Synchronize | Settings | Users
    expect(env.menuLength).toEqual(5);
    env.selectItem(0);

    // Translate | Overview | Generate Draft | Matthew | Mark | Community Checking | Synchronize | Settings | Users
    expect(env.menuLength).toEqual(8);

    env.allowUserToSeeGenerateDraft();
    tick();
...
class TestEnvironment {
  ...
  constructor(initialConnectionStatus?: 'online' | 'offline', isLoggedIn: boolean = true) {
    ...
    when(mockedGenerateDraftAuthGuard.allowTransition(anything())).thenReturn(this.canSeeGenerateDraft$);
    ...
    when(mockedFeatureFlagService.showNmtDrafting).thenReturn({ enabled: false } as FeatureFlag);
    when(mockedFeatureFlagService.showFeatureFlags).thenReturn({ enabled: true } as FeatureFlag);
    when(mockedFeatureFlagService.stillness).thenReturn({ enabled: false } as FeatureFlag);
    when(mockedFeatureFlagService.showNonPublishedLocalizations).thenReturn({ enabled: false } as FeatureFlag);
