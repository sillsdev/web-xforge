...
export class ResourceMonitor {
  private static _instance: ResourceMonitor;
  ...
  /** Listen for requests to record resource usage. */
  private async listen(): Promise<void> {
    try {
      const fifoPath: string = await this.createFifo();
      this.readFifo(fifoPath);
    } catch (err) {
      console.error('Ignoring error setting up ResourceMonitor FIFO listener:', err);
    }
  }

  private async createFifo(): Promise<string> {
    const dir: string = process.env.XDG_RUNTIME_DIR ?? process.env.TEMP ?? '/tmp';
    const filename: string = 'sf-res-mon.fifo';
    const fifoPath: string = path.join(dir, filename);
    try {
  ...
  /** Respond to $ echo 'record-resource-usage' > ${XDG_RUNTIME_DIR}/sf-res-mon.fifo */
  private readFifo(fifoPath: string): void {
    try {
      const readStream: fs.ReadStream = fs.createReadStream(fifoPath, { encoding: 'utf8' });

      readStream.on('data', (data: string) => {
        if (data.trim() === 'record-resource-usage') this.record();
      });

      // Keep re-opening the stream after a writer closes.
      readStream.on('end', () => {
        setImmediate(() => this.readFifo(fifoPath));
      });

      readStream.on('error', (err: Error) => {
        console.error('Ignoring FIFO read error:', err);
      });
    } catch (err) {
      console.error('Ignoring error creating FIFO read stream:', err);
    }
  }
}
