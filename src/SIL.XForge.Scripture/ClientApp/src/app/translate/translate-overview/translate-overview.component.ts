  ...
  private async updateTextProgress(project: SFProjectProfileDoc, textProgress: TextProgress): Promise<void> {
    for (const chapter of textProgress.text.chapters) {
      ...
      if (
        ...
        // Get the intersect of the source and target translated verses arrays
        // i.e. The verses with data that both texts have in common
        const targetTranslatedVerses: string[] = chapterText.getTranslatedVerses();
        const sourceTranslatedVerses: string[] = sourceChapterText.getTranslatedVerses();
        this.segmentPairs += targetTranslatedVerses.filter(item => sourceTranslatedVerses.includes(item)).length;
      }
    }
  }
