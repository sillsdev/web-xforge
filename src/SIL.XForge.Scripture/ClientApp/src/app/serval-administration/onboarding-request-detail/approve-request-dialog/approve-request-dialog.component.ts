...
function languageCodesValidator(languageCodes: Map<string, string>) {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    const codes = allIds.map(id => languageCodes.get(id)).filter((c): c is string => c != null && c !== '');
...
export class ApproveRequestDialogComponent {
  ...
  constructor(
    ...
    this.backTranslationLanguageMatchesTarget =
      ...
      normalizeLanguageCodeToISO639_3(data.backTranslation.languageCode) ===
        normalizeLanguageCodeToISO639_3(data.targetProject.languageCode);
  ...
  get sourcesMatchTargetLanguage(): boolean {
    ...
    const codes = allIds.map(id => this.languageCodes.get(id)).filter((c): c is string => c != null && c !== '');
    if (codes.length === 0) return false;
    const uniqueNormalized = new Set(codes.map(normalizeLanguageCodeToISO639_3));
    if (uniqueNormalized.size !== 1) return false;
    return [...uniqueNormalized][0] === normalizeLanguageCodeToISO639_3(this.data.targetProject.languageCode);
  }
