...
namespace UpdateHelp
{
    class Program
    {
        ...
        /// Run this program, <c>dotnet run write</c>
        /// Push to help AWS S3:
        /// <code>
        ...
        {
            // change this for different targets
            string target = "es";

            bool doWrite = ((args.Length >= 1 ? args[0] : "") == "write");
        ...
        static void TestModeWarning()
        {
            ...
            Console.WriteLine(" - no files are changed. Run `UpdateHelp write` to change files.\n");
        }
        ...
        static Dictionary<string, string> ExtractHtmlTranslations(string sourcePath, string targetPath)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"The are {files.Length} HTML source files.");
                string heading;
                string relativeFile;
                foreach (string file in files)
                {
                    relativeFile = Path.GetRelativePath(sourcePath, file);
                    heading = GetHtmlHeading(file);
                    if (heading != "")
                        sourceFileToHeadings.Add(relativeFile, heading);
                }

                files = Directory.GetFiles(targetPath, "*.htm", SearchOption.AllDirectories);
                foreach (string file in files)
                {
                    relativeFile = Path.GetRelativePath(targetPath, file);
                    heading = GetHtmlHeading(file);
                    if (heading != "" && sourceFileToHeadings.ContainsKey(relativeFile))
                        result.Add(sourceFileToHeadings[relativeFile], heading);
                }
            }
            catch (Exception e)
            {
            ...
            string targetMenuPath)
        {
            ...
            using (JsonDocument targetMenuDoc = JsonDocument.Parse(targetMenuJson))
            {
                ...
                foreach (var targetMenuItem in targetMenuItems)
                    if (sourceMenuItems.ContainsKey(targetMenuItem.Key))
                        translations.TryAdd(sourceMenuItems[targetMenuItem.Key], targetMenuItem.Value);
            }
        }
            ...
            bool doWrite)
        {
            try
            {
                ...
                Console.WriteLine($"The are {files.Length} idata source files.");
                XDocument doc;
                ...
                foreach (string file in files)
                {
                    doc = XDocument.Parse(GetBufferXmlFromFile(file));
            ...
            bool doWrite)
        {
            try
            {
                ...
                Console.WriteLine($"The are {files.Length} toc source files.");
                XDocument doc;
            ...
            Dictionary<string, string> attr2ByAttr1)
        {
            ...
            foreach (XElement element in elementList)
            {
                ...
                if (attr2ByAttr1.ContainsKey(attr1))
                {
                    if (attr2ByAttr1[attr1] == attr2)
                        return;
            ...
            Dictionary<string, string>.KeyCollection sources, Dictionary<string, string> translations)
        {
            ...
            foreach (string source in sources)
            {
                if (translations.ContainsKey(source))
                    text = text.Replace(textExpression(source), textExpression(translations[source]));
            }
