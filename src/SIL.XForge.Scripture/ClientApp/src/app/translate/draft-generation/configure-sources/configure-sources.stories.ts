...
async function getSourceElement(
  ...
  const trainingTargetWrapper = canvasElement.querySelectorAll('.inputs-source-side')[1] as HTMLElement;
  if (fieldName === 'training_a') {
    ...
    return await within(trainingTargetWrapper).findByRole('combobox');
