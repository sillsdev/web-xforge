...
describe('SyncProgressComponent', () => {
  ...
  it('does not initialize if projectDoc is undefined', fakeAsync(async () => {
    ...
    verify(mockedProjectService.subscribe('sourceProject02', FETCH_WITHOUT_SUBSCRIBE)).never();
...
class TestEnvironment {
  ...
  constructor(args: TestEnvArgs) {
    ...
    when(mockedProjectService.subscribe('testProject01', FETCH_WITHOUT_SUBSCRIBE)).thenCall(() =>
      this.realtimeService.subscribe(SFProjectDoc.COLLECTION, 'testProject01', FETCH_WITHOUT_SUBSCRIBE)
    );
    when(mockedProjectService.subscribe('sourceProject02', FETCH_WITHOUT_SUBSCRIBE)).thenCall(() =>
