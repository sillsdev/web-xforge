...
export class TextChooserDialogComponent implements OnDestroy {
  ...
  private expandSelection(selection: Selection, segments: Element[]) {
    ...
    const startOffset = this.isInSegment(startRange.startContainer) ? startRange.startOffset : 0;
    const endOffset = this.isInSegment(endRange.endContainer)
      ? endRange.endOffset
