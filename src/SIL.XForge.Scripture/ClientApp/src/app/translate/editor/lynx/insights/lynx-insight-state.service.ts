...
import { LynxService } from './lynx-workspace.service';
...
export class LynxInsightStateService {
  ...
  private rawInsightSource$: Observable<LynxInsight[]> = this.lynxService.workspace.diagnosticsChanged$.pipe(
    switchMap(async e => {
      if (e.diagnostics.length === 0) {
        this.curInsights.delete(e.uri);
      } else {
        const doc = await this.lynxService.documentManager.get(e.uri);
  ...
  constructor(
    ...
    private readonly lynxService: LynxService
  ) {
    this.init();
  }
