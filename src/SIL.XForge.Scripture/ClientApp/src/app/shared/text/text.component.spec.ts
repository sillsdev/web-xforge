...
describe('TextComponent', () => {
  ...
  it('does not cancel in beforeinput when valid selection', fakeAsync(() => {
    const { env }: { env: TestEnvironment; segmentRef: string } = basicSimpleText();
...
function basicSimpleText(): { env: TestEnvironment; segmentRef: string } {
  const chapterNum = 2;
  const segmentRef: string = `verse_${chapterNum}_1`;
  const textDocOps: RichText.DeltaOperation[] = [
    { insert: { chapter: { number: chapterNum.toString(), style: 'c' } } },
  ...
  env.component.editor?.setSelection(segmentRange.index);
  env.fixture.detectChanges();
  tick();
  return { env, segmentRef };
}
