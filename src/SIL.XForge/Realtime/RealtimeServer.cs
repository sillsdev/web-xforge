...
public class RealtimeServer : IRealtimeServer
{
    ...
    public Task DeleteDocAsync(int handle, string collection, string id) =>
        InvokeExportAsync<object>("deleteDoc", handle, collection, id);

    public void Disconnect(int handle) => InvokeExportAsync<object>("disconnect", handle).GetAwaiter().GetResult();

    public Task DisconnectAsync(int handle) => InvokeExportAsync<object>("disconnect", handle);
