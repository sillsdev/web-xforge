...
```bash
sudo perf record --pid=$(pgrep SIL.XForge.Scri) -g --timestamp-filename # Then press Ctrl+C when done. Optionally also use --output perf.data-my-report-filename
...
```bash
dotnet-trace collect --process-id $(pgrep SIL.XForge.Scri) --format chromium
```
