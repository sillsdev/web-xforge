...
export class DraftRequestDetailComponent extends DataLoadingComponent implements OnInit {
  ...
  get warnings(): string[] {
    ...
    if (this.request?.submission.formData.partnerOrganization !== 'none' && this.request?.resolution !== 'outsourced') {
      warnings.push('This request has a partner organization specified but is not yet marked as outsourced.');
    }

    const projectISOCode = this.mainProjectDoc?.data?.writingSystem.tag;
    const formISOCode = this.request?.submission.formData.translationLanguageIsoCode?.trim();
    if (
      projectISOCode &&
      formISOCode &&
    ...
    for (const [paratextId, projectDoc] of this.projectDocs.entries()) {
      if (projectDoc.data?.sync.lastSyncSuccessful === false) {
        const projectName = this.projectNames.get(paratextId) ?? paratextId;
