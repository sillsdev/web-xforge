...
export class ShareControlComponent extends SubscriptionDisposable {
  ...
  get defaultShareRole(): string | undefined {
    ...
    if (this.defaultRole != null && roles.some(r => r.role === this.defaultRole)) {
  ...
  get roles(): ProjectRoleInfo[] {
    ...
    return SF_PROJECT_ROLES.filter(info => info.canBeShared && this.availableRoles.includes(info.role));
  }
  ...
  get isLinkSharingEnabled(): boolean {
    ...
    return this.availableRoles.includes(this.shareRole) && shareLevelSettings[this.shareRole] === true;
  }
