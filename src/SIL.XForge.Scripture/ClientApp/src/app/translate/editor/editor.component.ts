...
export class EditorComponent extends DataLoadingComponent implements OnDestroy, AfterViewInit {
  ...
  private async updateSegmentNoteThreadAnchors(
    oldSegmentEmbeds: Map<string, number>,
    newEditorChange: DeltaStatic
  ): Promise<void> {
    if (this.noteThreadQuery == null || this.noteThreadQuery.docs.length < 1) {
      return;
    }
    const updatePromises: Promise<boolean>[] = [];
    if (newEditorChange.ops == null || newEditorChange.ops.length < 2) {
      // All editor changes we want to process will begin with a retain, followed by at least another op.
      console.warn(
        `Warning: updateSegmentNoteThreadAnchors: new editor change had ops null or length less than two:`,
        newEditorChange.ops
      );
      return;
    }
    const editOpIndex: number | undefined = newEditorChange.ops[0].retain;
    let length = 0;
    let operation: 'insert' | 'delete' = 'insert';
    // get the length that was inserted or deleted to apply to the note text anchor
    if (newEditorChange.ops[1].insert != null && typeof newEditorChange.ops[1].insert === 'string') {
      length = newEditorChange.ops[1].insert.length;
    } else if (newEditorChange.ops[1].delete != null) {
      length = newEditorChange.ops[1].delete;
      operation = 'delete';
    }
