...
public class ParatextService : DisposableBase, IParatextService
{
    ...
    )
    {
        ...
        void SyncMetricsLog(SyncMetrics syncMetrics, string message) =>
            syncMetrics?.Log.Add($"{DateTime.UtcNow:u} SR: {message}");
        void traceListener(string message) => SyncMetricsLog(syncMetrics, $"Trace {message}");
        void alertListener(string message) => SyncMetricsLog(syncMetrics, $"Alert {message}");
        ...
        try
        {
            ...
            if (TryGetProject(userSecret, sendReceiveRepository, projectsMetadata, out ParatextProject ptProject))
            {
                if (!projectGuids.Contains(paratextId))
                {
                    SyncMetricsLog(
                        syncMetrics,
                        $"The project with PT ID {paratextId} did not have a full name available."
                    );
                }
            }
            else
            ...
            if (ptProject is not ParatextResource)
            {
                ...
                void SyncMetricsLogCommitGraph()
                {
                    ...
                    SyncMetricsLog(syncMetrics, $"Commit graph:");
                    SIL.Linq.Enumerable.ForEach(
                        SIL.Extensions.StringExtensions.SplitLines(graph),
                        line => SyncMetricsLog(syncMetrics, line)
                    );
                }
                ...
                SyncMetricsLog(syncMetrics, srResultDescriptions);
                SyncMetricsLogCommitGraph();
                if (
                    !noErrors
                    || !success
