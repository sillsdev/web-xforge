...
) : IMachineProjectService
{
    ...
    )
    {
        ...
        catch (TaskCanceledException e) when (e.InnerException is not TimeoutException)
        {
            ...
            // Ensure that the queued at timestamp is not present, if we are pre-translating
            if (preTranslate)
            {
                await projectSecrets.UpdateAsync(
                    buildConfig.ProjectId,
        ...
        {
            // A build is already in progress - clear the job details and don't record the error, if pre-translating
            if (preTranslate)
            {
                await projectSecrets.UpdateAsync(
                    buildConfig.ProjectId,
        ...
        catch (Exception e)
        {
            ...
            // Update the project secret with the error message, if we are pre-translating
            if (preTranslate)
            {
                await projectSecrets.UpdateAsync(
                    buildConfig.ProjectId,
    ...
    )
    {
        ...
        corpusUpdated |= await UpdateCorpusConfigAsync(
            ...
            uploadParatextZipFile,
            corpusUpdated,
            sourceCorpusFiles: newSourceCorpusFiles,
            targetCorpusFiles: newTargetCorpusFiles,
            cancellationToken
        );
    ...
    /// <param name="sourceProjectId">The source project identifier (this may be a mix source or a training source).</param>
