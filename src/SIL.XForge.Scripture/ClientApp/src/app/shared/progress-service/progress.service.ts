...
export class ProgressService {
  ...
  // Alongside its age, every cache entry and in-flight request records the project's sync token from when it was
  // fetched/started, so data from before the latest sync can be recognized as stale regardless of age.
  private projectProgressCache = new Map<
    string,
    { timestampMs: number; syncToken: string | undefined; progress: ProjectProgressWithChapterProgress }
  >();
  private requestCache = new Map<
    string,
    { startedAtMs: number; syncToken: string | undefined; promise: Promise<ProjectProgressWithChapterProgress> }
  >();

  /**
   * `maxStalenessMs` is how old cached data may be and still be returned. Independent of age, data from before the
   * project's most recent successful sync never qualifies: entries are stamped with the project's sync token
   * (`sync.dateLastSuccessfulSync`) and served only while the token is unchanged. Tokens are compared for
   * equality, never against the clock, so server/client clock skew cannot break this. The age window exists for what
   * a sync token can't see: progress drift from live editing in Scripture Forge itself.
   */
  async getProgressWithChapterProgress(
    ...
    const syncToken: string | undefined = projectDoc.data?.sync?.dateLastSuccessfulSync;
    const qualifies = (timestampMs: number, token: string | undefined): boolean =>
      Date.now() - timestampMs < options.maxStalenessMs && token === syncToken;

    const cachedProgress = this.projectProgressCache.get(projectId);
    if (cachedProgress != null && qualifies(cachedProgress.timestampMs, cachedProgress.syncToken)) {
    ...
    // freshness requirements. Sibling reads issued together share one request (same token), while a request started
    // before the latest sync never qualifies.
    const existingRequest = this.requestCache.get(projectId);
    if (existingRequest != null && qualifies(existingRequest.startedAtMs, existingRequest.syncToken)) {
    ...
    const requestPromise = this.projectService
      ...
      .then(bookProgressList => {
        ...
        if (cacheEntry == null || cacheEntry.timestampMs <= requestTimestamp) {
          this.projectProgressCache.set(projectId, { timestampMs: requestTimestamp, syncToken, progress });
        }
    ...
    this.requestCache.set(projectId, { startedAtMs: requestTimestamp, syncToken, promise: requestPromise });
