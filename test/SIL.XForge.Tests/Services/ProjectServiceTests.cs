...
namespace SIL.XForge.Services
{
    ...
    public class ProjectServiceTests
    {
        ...
        [Test]
        public async Task RemoveUserAsync_NoRaceFailure()
        {
            var env = new TestEnvironment();
            string requestingUser = User01;
            string projectA = Project01;
            string projectB = Project02;
            string userToDisassociate = User02;
            Assert.That(env.GetProject(projectA).UserRoles, Does.ContainKey(userToDisassociate), "setup");
            Assert.That(env.GetProject(projectB).UserRoles, Does.ContainKey(userToDisassociate), "setup");
            Assert.That(env.GetUser(userToDisassociate).Sites[SiteId].Projects, Does.Contain(projectA), "setup");
            Assert.That(env.GetUser(userToDisassociate).Sites[SiteId].Projects, Does.Contain(projectB), "setup");
            var tasks = new List<Task>();
            // SUT
            // Two simultaneous requests to disassociate a user, each regarding a different project.
            tasks.Add(env.Service.RemoveUserAsync(requestingUser, projectA, userToDisassociate));
            tasks.Add(env.Service.RemoveUserAsync(requestingUser, projectB, userToDisassociate));
            await Task.WhenAll(tasks);

            Assert.That(env.GetProject(projectA).UserRoles, Does.Not.ContainKey(userToDisassociate));
            Assert.That(env.GetProject(projectB).UserRoles, Does.Not.ContainKey(userToDisassociate));
            Assert.That(env.GetUser(userToDisassociate).Sites[SiteId].Projects, Does.Not.Contain(projectA));
            Assert.That(env.GetUser(userToDisassociate).Sites[SiteId].Projects, Does.Not.Contain(projectB));
        }
