...
namespace SIL.XForge.Scripture.Services
{
    ...
    public class ParatextServiceTests
    {
        ...
        public async Task GetNoteThreadChanges_RetrievesChanges()
        {
            ...
            using (IConnection conn = await env.RealtimeService.ConnectAsync())
            {
                ...
                Assert.That(
                    ...
                    Is.EqualTo(
                        "Context before Text selected thread6 context after.-Start:15-Length:21-MAT 1:6-conflict1"
                    )
                );
        ...
        public void SetRepoToRevision_SetsAndChecks()
        {
            ...
            env.MockHgWrapper.GetRepoRevision(Arg.Any<string>()).Returns(rev);
            // SUT
        ...
        },
        {
            ...
            public void AddNoteThreadData(ThreadComponents[] threadComponents)
            {
                ...
                foreach (var comp in threadComponents)
                {
                    ...
                    string threadIcon =
                        comp.notes == null ? "icon1" : $"icon{comp.notes[comp.notes.Length - 1].tagsAdded[0]}";
