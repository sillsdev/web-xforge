...
describe('NllbLanguageService', () => {
  ...
  describe('isLanguageSupportedAsync', () => {
    ...
    it('should fallback to isNllbLanguage on client error', fakeAsync(async () => {
    ...
    it('should fallback to isNllbLanguage on server error', async () => {
      httpClient.get = jasmine.createSpy().and.returnValue(of({ status: HttpStatusCode.GatewayTimeout }));
      expect(await service.isNllbLanguageAsync('en')).toBe(true);
    });
  });
