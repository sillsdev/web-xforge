...
import en_checking from '../assets/i18n/checking_en.json';
import en_non_checking from '../assets/i18n/non_checking_en.json';
...
export const en = merge(en_checking, en_non_checking);
...
export class TranslationLoader implements TranslocoLoader {
  ...
  getTranslation(code: string) {
    if (code === 'en') {
      ...
      return zip(
        this.http.get<Translation>(`/assets/i18n/checking_${code}.json`).toPromise(),
        this.http.get<Translation>(`/assets/i18n/non_checking_${code}.json`).toPromise()
