...
class TestEnvironment {
  ...
  constructor() {
    ...
    when(mockedServalAdministrationService.get).thenResolve(undefined);
    when(mockedOnboardingRequestService.getAllRequests()).thenResolve([]);
...
function createTestRequest(
  ...
  return {
    ...
    submittedBy: { name: overrides.name ?? 'Requester', email: overrides.email ?? 'requester@example.com' },
    submission: {
      projectId: 'project01',
      userId: 'user03',
      timestamp,
      formData: {
        ...
        completedBooks: [40],
        nextBooksToDraft: [41],
        sourceProjectA: 'source_project',
        draftingSourceProject: 'source_project',
