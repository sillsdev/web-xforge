...
describe('CommandService', () => {
  ...
  it('handles response with JsonRpcError', fakeAsync(() => {
    // A way to fail if the server provides a JsonRpcResponse with an JsonRpcError. This might have been the expected error information format, but it's not clear that we actually use this anymore.
  ...
  it('handles failure status', fakeAsync(() => {
    // A way to fail that just has an error status. Like an example at https://angular.io/guide/http#testing-for-errors . We might get this kind of error when we have a programming mistake in our application, for example, perhaps by calling an API at a very wrong address.
    ...
    env.service
      ...
      .catch((errorInfo: any) => {
        ...
        // Command error code should not be set to a normal http status code like 404, since that is not a CommandErrorCode.
  ...
  it('handles ErrorEvent', fakeAsync(() => {
    // A way to fail without actually reaching the server. Like an example at https://angular.io/guide/http#testing-for-errors . This kind of error seems likely, as a result of networking trouble. Though in testing, if http.post() happens when the Internet is off, there is an HttpErrorResponse with status 0 and statusText "Unknown Error".
  ...
  // It would be nice to have a test that gets down to the fall-thru condition of a type of error that didn't match what we expected. But the HttpTestingController isn't letting a request.error() be done that is not an ErrorEvent. And if we request.flush(jsonRpcResponse), the jsonRpcResponse.error must be a JsonRpcError.
});
