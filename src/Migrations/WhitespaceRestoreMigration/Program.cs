...
///   export SF_APP_DIR=/path/to/sf/app; sudo --user $(stat --format='%G' "${SF_APP_DIR}") ASPNETCORE_ENVIRONMENT=DevelopmentORStagingORProduction time ./WhitespaceRestoreMigration dry-run "${SF_APP_DIR}" |& tee ~/output-WhitespaceRestoreMigration-$(date '+%Y%m%d-%H%M%S').txt
/// </summary>
