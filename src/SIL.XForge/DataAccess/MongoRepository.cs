...
{
    public class MongoRepository : IRepository<T> where T : IIdentifiable
    {
        ...
        public async Task InsertAsync(T entity)
        {
            ...
            {
                if (e.WriteError.Category == ServerErrorCategory.DuplicateKey) throw new DuplicateKeyException(e);
                throw;
            }
        }
        ...
        public async Task<bool> ReplaceAsync(T entity, bool upsert = false)
        {
            try
            {
                ReplaceOneResult result =
                    await _collection
                        .ReplaceOneAsync(e => e.Id == entity.Id, entity, new UpdateOptions { IsUpsert = upsert });
                if (result.IsAcknowledged) return upsert || result.MatchedCount > 0;
                return false;
            }
            catch (MongoWriteException e)
            {
                if (e.WriteError.Category == ServerErrorCategory.DuplicateKey) throw new DuplicateKeyException();
                throw;
            }
        }
