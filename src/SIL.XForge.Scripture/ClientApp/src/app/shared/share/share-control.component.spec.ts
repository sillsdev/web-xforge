...
describe('ShareControlComponent', () => {
  ...
  it('share link should not be shown when offline', fakeAsync(() => {
    ...
    expect(env.linkSharingOfflineMessageText).toContain('Inviting users by link sharing is unavailable while offline.');
  }));
...
class TestEnvironment {
  ...
  get linkSharingOfflineMessageText(): string {
    return this.elementText(this.fetchElement('.invite-by-link .offline-text'));
  }
