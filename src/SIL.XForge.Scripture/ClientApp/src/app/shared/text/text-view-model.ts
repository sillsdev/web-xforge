...
export class TextViewModel {
  ...
  private fixSegment(segment: SegmentInfo, fixDelta: DeltaStatic): DeltaStatic {
    if (segment.length === 0) {
      ...
      delta.insert({ blank: type }, { segment: segment.index });
      fixDelta = fixDelta.compose(delta);
    } else if (segment.containsBlank && segment.length > 1) {
      // delete blank
      const delta = new Delta()
        .retain(segment.index)
        .retain(segment.length - 1, { segment: segment.ref })
        .delete(1);
      fixDelta = fixDelta.compose(delta);
    } else if (segment.ref !== segment.origRef) {
      // fix segment ref
      const delta = new Delta().retain(segment.index).retain(segment.length, { segment: segment.ref });
      fixDelta = fixDelta.compose(delta);
    }
    return fixDelta;
  }
