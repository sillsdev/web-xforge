  ...
  describe('tabs', () => {
    ...
    describe('updateAutoDraftTabVisibility', () => {
      ...
      it('should not add draft preview tab when draft formatting (usfmConfig) is not set', fakeAsync(() => {
        ...
        when(mockedDraftOptionsService.shouldBlockDraftTabDueToFormattingOptions(anything())).thenReturn(true);
        when(mockedPermissionsService.canAccessDrafts(anything(), anything())).thenReturn(true);
        env.wait();
        env.routeWithParams({ projectId: 'project01', bookId: 'LUK', chapter: '1' });
        env.wait();
...
class TestEnvironment {
  ...
  constructor(preInit?: (env: TestEnvironment) => void) {
    ...
    when(mockedDraftOptionsService.shouldBlockDraftTabDueToFormattingOptions(anything())).thenReturn(false);
