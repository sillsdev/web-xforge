...
class JiraGitHubCorrelator {
  ...
  private async fetchJiraIssues(status: string): Promise<JiraIssue[]> {
    ...
    if (!this.jiraPersonalAccessToken) {
      console.warn('JIRA_ACCESS_TOKEN environment variable not set. Skipping Jira data fetch.');
      return [];
    }
  ...
  private async fetchGitHubPullRequests(): Promise<GitHubPullRequest[]> {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'Jira-GitHub-Correlator/1.0'
      };
    ...
    try {
      ...
      for (const review of reviews) {
        if (review.user && review.state !== 'DISMISSED') {
          ...
          if (
            !latestReviews.has(userId) ||
            new Date(review.submitted_at) > new Date(latestReviews.get(userId).submitted_at)
          ) {
      ...
      // Check the latest review state from each reviewer
