...
export class TextComponent extends SubscriptionDisposable implements AfterViewInit, OnDestroy {
  ...
  embedElementInline(
    ...
    if (Array.from(this.viewModel.embeddedElements.keys()).includes(id)) {
      console.warn(`Warning: embedElementsInline: Not re-embedding existing embed id ${id} in verse:`, verseRef);
      return;
    }

    let segmentToModify: string = verseSegments[0];
  ...
  /** Returns the number of embedded elements that are located at or after editorStartPos, thru length of editor
   * positions to check.
   */
  private getEmbedCountInRange(editorStartPos: number, length: number): number {
    ...
    return Array.from(embedPositions.values()).filter(
      (value: number) => value >= editorStartPos && value < editorStartPos + length
    ).length;
  }
