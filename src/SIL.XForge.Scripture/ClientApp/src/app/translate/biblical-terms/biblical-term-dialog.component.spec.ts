...
describe('BiblicalTermDialogComponent', () => {
  ...
  it('should display the biblical term', fakeAsync(() => {
    ...
    env.openDialog('id01');
    expect(env.term).toBe('termId01 --- gloss01_en --- notes01_en');
  ...
  it('should display biblical term in the specified language', fakeAsync(() => {
    ...
    env.openDialog('id01');
    expect(env.term).toBe('termId01 --- gloss01_fr --- notes01_fr');
    expect(env.renderings.value).toBe('rendering01');
    expect(env.description.value).toBe('description01');
    env.closeDialog();
  }));
  ...
  it('should display transliterations when enabled', fakeAsync(() => {
    ...
    env.openDialog('id01');
    expect(env.term).toBe('transliteration01 --- gloss01_en --- notes01_en');
    expect(env.renderings.value).toBe('rendering01');
    expect(env.description.value).toBe('description01');
    env.closeDialog();
  }));
...
class TestEnvironment {
  ...
  get term(): string | null {
    return this.overlayContainerElement.querySelector('#term')?.textContent ?? null;
  }
