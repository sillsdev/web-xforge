...
/// Archives.)
///
/// Scripture Forge must be running in order to run this migrator, which uses the existing realtime server. Note that "read" mode only doesn't write a migration. It does touch the local hg repo, and even disable sync on all projects.
///
