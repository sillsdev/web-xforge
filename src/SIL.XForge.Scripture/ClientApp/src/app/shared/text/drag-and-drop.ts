...
export class DragAndDrop {
  ...
  /**
   * Determine the editor length for `node` up until a contained `terminationChildNode`. The only lengths reported in
   * this way are the lengths of text nodes and known amounts for specific elements.
   */
  private determineLengthBeforeTerminationNode(node: Node, terminationChildNode: Node): number {
    ...
    if (node.nodeName.toLowerCase() === 'display-note') {
      const lengthForDisplayNoteElement = 1;
      return lengthForDisplayNoteElement;
    }
    if (node.nodeType === Node.TEXT_NODE) {
      if (node.nodeValue == null) {
        throw new Error('Did not expect a text node to have a null value.');
      }
