...
class TestEnvironment {
  ...
  /** Value in DOM from 'Show n more unread answers' banner in answer component. */
  get unreadAnswersBannerCount(): number {
    const valueInDom = document.querySelector('#show-unread-answers-count');
    if (valueInDom == null || valueInDom.textContent == null) {
      return -1;
    }
    const count = parseInt(valueInDom.textContent, 10);
    if (count == null) {
      return -1;
    }
    return count;
  ...
  /** Value in DOM from 'n Answers' heading in answer component. */
  get totalAnswersMessageCount(): number {
    const valueInDom = document.querySelector('#total-answers-message-count');
    if (valueInDom == null || valueInDom.textContent == null) {
      return -1;
    }
    const count = parseInt(valueInDom.textContent, 10);
