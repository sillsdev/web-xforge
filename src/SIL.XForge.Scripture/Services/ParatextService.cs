...
public class ParatextService : DisposableBase, IParatextService
{
    ...
    private readonly string _dblServerUri = "https://paratext.thedigitalbiblelibrary.org/";
    ...
    private IEnumerable<CommentThread> GetCommentThreads(UserSecret userSecret, string projectId, int bookNum)
    {
        ...
        IEnumerable<CommentThread> threads = manager.FindThreads(
            (commentThread) => commentThread.VerseRef.BookNum == bookNum,
            false
        );
...
{
    private readonly SemaphoreSlim _userSemaphore;
