...
export class VerseRef {
   ...
   * @returns boolean: True if the specified string was successfully parsed, false otherwise
   * @returns VerseRef: The result of the parse if successful, or empty VerseRef if it failed
   */
  static tryParse(str: string): VerseRef | undefined {
    let vref: VerseRef;
    try {
      vref = VerseRef.parse(str);
      return vref;
    } catch (error) {
