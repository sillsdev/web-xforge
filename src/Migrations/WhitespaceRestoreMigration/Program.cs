...
/// Usage: cd bin/Debug/net6.0; ASPNETCORE_ENVIRONMENT=Development ./WhitespaceRestoreMigration read-mode ../../../../../SIL.XForge.Scripture
/// Usage on server: Shut down the SF service, and then run:
///   export SF_APP_DIR=/path/to/sf/app; sudo --user $(stat --format='%G' "${SF_APP_DIR}") ASPNETCORE_ENVIRONMENT=DevelopmentORStagingORProduction time ./WhitespaceRestoreMigration read-mode "${SF_APP_DIR}" |& tee ~/output-WhitespaceRestoreMigration-$(date '+%Y%m%d-%H%M%S').txt
/// </summary>
public class Program
{
    ...
    {
        string usage = "Arg 1 must be 'read-mode' or 'write-mode'. Arg 2 must be the path to the SF app dir.";
        if (args.Length < 2)
            throw new Exception($"Error: Missing required arguments. {usage}");
        bool writeMode;
        if (args[0] == "read-mode")
            writeMode = false;
        else if (args[0] == "write-mode")
            writeMode = true;
        else
        ...
        Log($"Running in {(writeMode ? "write-mode. An SF instance had better not also be running!" : "read-mode.")}");
        ...
        bool success = await migrator!.MigrateAsync(writeMode, sfProjectIdsToMigrate, sfUsersToUse);
