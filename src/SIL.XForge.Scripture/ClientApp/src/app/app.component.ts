...
export class AppComponent extends DataLoadingComponent implements OnInit, OnDestroy {
  ...
  private async refreshQuestionsQuery(): Promise<void> {
    this.questionsQuery?.dispose();
    if (this.selectedProjectDoc == null) return;

    const query = await this.projectService.queryQuestions(this.selectedProjectDoc?.id, { activeOnly: true });
    this.questionsQuery = query;
    query.docs$.subscribe(docs => {
