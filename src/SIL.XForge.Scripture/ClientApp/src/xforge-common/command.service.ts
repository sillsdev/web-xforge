...
export class CommandService {
  ...
  async onlineInvoke<T>(url: string, method: string, params: any = {}): Promise<T | undefined> {
    ...
    try {
      ...
      if (
        // Does error.error implement ErrorEvent?
        error.error != null &&
        ...
        const errorEvent: ErrorEvent = error.error; // as ErrorEvent;
        ...
        // Does error implement JsonRpcError?
        // It's not clear if we use JsonRpcError anymore.
