  ...
  constructor(quill: Quill, options: DragAndDropOptions) {
    ...
    quill.container.addEventListener('drop', (event: Event) => {
      ...
      if (dragEvent.dataTransfer == null) {
        console.log(`Warning: DragEvent unexpectedly has null dataTransfer property.`);
        return;
      }
      ...
      const destanationSegmentRange: RangeStatic | undefined =
        options.textComponent.getSegmentRange(destinationSegmentRef);
      if (destanationSegmentRange == null) {
        console.log('Warning: drag-and-drop destination segment had an undefined range.');
        return;
      }
      ...
      if (document.caretRangeFromPoint !== undefined) {
        ...
        if (range == null) {
          console.log('Warning: drag-and-drop inferred a null caret position for insertion.');
          return;
        }
        startPositionInSegment = range.offset;
      } else {
        console.log(`Warning: Could not determine insertion position for drag-and-drop.`);
        return;
      }

      const startingPositionInDocument: number = destanationSegmentRange.index + startPositionInSegment;
