...
describe('SFProjectMigrations', () => {
  ...
  describe('version 2', () => {
    it('migrates docs', async () => {
      ...
      expect(projectDoc.data.texts[0].permissions['user02']).toBe(TextInfoPermission.Read);
      expect(Object.keys(projectDoc.data.texts[0].permissions).length).toBe(2);
      expect(projectDoc.data.texts[0].sourcePermissions['user01']).toBe(TextInfoPermission.Read);
      expect(projectDoc.data.texts[0].sourcePermissions['user02']).toBe(TextInfoPermission.None);
      expect(Object.keys(projectDoc.data.texts[0].sourcePermissions).length).toBe(2);
      expect(projectDoc.data.texts[1].permissions['user01']).toBe(TextInfoPermission.Write);
      expect(projectDoc.data.texts[1].permissions['user02']).toBe(TextInfoPermission.Read);
      expect(Object.keys(projectDoc.data.texts[1].permissions).length).toBe(2);
