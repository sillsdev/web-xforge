...
export class DraftViewerComponent extends SubscriptionDisposable implements OnInit {
  ...
  // This is so the source is hidden when it is missing (like the translate editor).
  // Note: When an alternate source is specified, that is used instead, so the source is then just used for display.
  bookHasSource = false;
