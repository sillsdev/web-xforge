...
import { SourceQuestion } from './import-questions-dialog.component';
...
describe('ImportQuestionsDialogComponent', () => {
  ...
  it('shows questions only from the books in the project', fakeAsync(() => {
    ...
    env.click(env.importFromTranscelerator);
    ...
    env.click(env.getButtonByText('Cancel'));
  }));
  ...
  it('select all selects and deselects all visible questions', fakeAsync(() => {
    ...
    expect(env.component.filteredList[0].checked).toBe(true);
    env.click(env.getButtonByText('Show All'));
    expect(env.selectAllCheckbox.checked).toBe(false);
    expect(env.selectAllCheckbox.indeterminate).toBe(true);
    env.clickSelectAll();
    expect(env.component.filteredList[0].checked).toBe(true);
  ...
  it('show scripture chooser dialog', fakeAsync(() => {
    ...
    verify(env.mdcDialogSpy.open(anything(), anything())).once();
  ...
  it('allows updates questions that have been edited in Transcelerator', fakeAsync(() => {
    const env = new TestEnvironment(true);
    when(env.mockedImportQuestionsConfirmationMdcDialogRef.afterClosed()).thenReturn(
      of({
        questions: [
          { before: 'Before', after: 'After', checked: true },
          { before: 'Before', after: 'After', checked: true }
        ]
      } as ImportQuestionsConfirmationDialogData)
    );
  ...
  it('should inform the user when Transcelerator version is unsupported', fakeAsync(() => {
    ...
    env.click(env.getButtonByText('Close'));
  }));
  ...
  it('allows canceling the import of questions', fakeAsync(() => {
    ...
    env.getButtonByText('Cancel').click();
    tick(12000);
    verify(mockedProjectService.createQuestion('project01', anything(), undefined, undefined)).once();
  }));
  ...
  it('can import from a CSV file', async () => {
    ...
    expect(env.overlayContainerElement.querySelectorAll('table[mat-table] tbody tr').length).toBe(1);
    env.getButtonByText('Continue import').click();
    env.fixture.detectChanges();
    env.getButtonByText('Cancel').click();
  ...
  it('infinite scrolls questions', fakeAsync(() => {
    const questions = Array.from(Array(200), (_, i) => ({
      book: 'MAT',
      startChapter: '1',
      startVerse: i + 1 + '',
...
class TestEnvironment {
  ...
  mdcDialogSpy: MdcDialog;
  matDialogSpy: MatDialog;
  ...
  private questions: SourceQuestion[] = [
    {
      book: 'GEN',
      startChapter: '41',
      startVerse: '39',
  ...
  constructor(includeAllBooks = false, private errorOnFetchQuestions = false, sourceQuestions?: SourceQuestion[]) {
    if (sourceQuestions != null) {
      this.questions = sourceQuestions;
    }
    ...
    // Set up MatDialog mocking after it's already used above in creating the component.
    this.matDialogSpy = spy(this.component.matDialog);
    this.mdcDialogSpy = spy(this.component.mdcDialog);
    when(this.mdcDialogSpy.open(ScriptureChooserDialogComponent, anything())).thenReturn(
      instance<MdcDialogRef<ScriptureChooserDialogComponent>>(this.mockedScriptureChooserMdcDialogRef)
    );
    when(this.mdcDialogSpy.open(ImportQuestionsConfirmationDialogComponent, anything())).thenReturn(
  ...
  get importFromTranscelerator(): HTMLButtonElement {
  ...
  get importSelectedQuestionsButton(): HTMLElement {
    return this.getButtonWhere(buttonText => /import \d+ selected questions/i.test(buttonText));
  }

  getButtonByText(text: string): HTMLElement {
    return this.getButtonWhere(buttonText => buttonText.toLowerCase() === text.toLowerCase());
  }

  getButtonWhere(matches: (text: string) => boolean): HTMLElement {
    return Array.from(this.overlayContainerElement.querySelectorAll('button')).find(button =>
  ...
  private setupTransceleratorQuestions(): void {
    ...
    when(mockedProjectService.queryQuestions('project01')).thenResolve({
      ...
      docs: [
        ...
        {
          data: {
            text: 'Now the famine was severe in the land before edit.',
