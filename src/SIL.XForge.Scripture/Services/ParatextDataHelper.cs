...
public class ParatextDataHelper(IFileSystemService fileSystemService) : IParatextDataHelper
{
    ...
    /// </remarks>
    public async Task MigrateResourceIfRequiredAsync(ScrText scrText, LanguageId? overrideLanguage)
    {
        ...
        )
        {
            ...
            if (!string.IsNullOrEmpty(verseFilePath))
            {
                ...
                await CreateCustomVersificationFileAsync(verseFilePath, scrText.FullPath);
    ...
    /// </remarks>
    private async Task CreateCustomVersificationFileAsync(string verseFilePath, string projectPath)
    {
