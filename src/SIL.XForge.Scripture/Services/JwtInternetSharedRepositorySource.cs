...
public class JwtInternetSharedRepositorySource : InternetSharedRepositorySource, IInternetSharedRepositorySource
{
    ...
    /// </summary>
    internal (bool isRevOnServer, int serverRevCount, string serverLastRev) CheckPushRevisionOnServer(
    ...
    )
    {
        ...
        string serverLastRev = GetLastServerRevision(revisions);
    ...
    /// </summary>
    private static string GetLastServerRevision(JArray? revisions)
    {
        if (revisions?.Count > 0)
            return revisions[0]["id"]?.ToString() ?? "(unknown)";
        return "(none)";
    }
