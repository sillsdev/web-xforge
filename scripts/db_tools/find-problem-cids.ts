...
import { colored, colors, databaseConfigs, fetchSnapshotByVersion, useColor, visualizeOps } from './utils.js';
...
async function examine(
  ...
  const docs: Array<any> = await textOperationCollection
...
async function run() {
  ...
  try {
    ...
    const projectsInDB: Array<any> = await projectCollection.find().toArray();
    for (const projectDoc of projectsInDB) {
      ...
      for (const book of booksInProject) {
        ...
        for (const chapterNumber of (book.chapters as Array<any>).map((chap: any) => chap.number)) {
          await examine(conn, textOperationCollection, projectDoc, bookAbbr, chapterNumber);
        }
      }
    }
  } finally {
...
function opCharStyles(op: any): Array<{ style: string; cid: string }> {
  let styles: Array<{ style: string; cid: string }> = [];
...
function allOpCharStyles(ops: Array<any>): Array<{ opNumber: number; styles: Array<{ style: string; cid: string }> }> {
  const opsStyling: Array<{ opNumber: number; styles: Array<{ style: string; cid: string }> }> = [];
  for (let i = 0; i < ops.length; i++) {
    const op: any = ops[i];
    const styles: Array<{ style: string; cid: string }> = opCharStyles(op);
 ...
 *  style usages. */
function detectInconsistentCids(ops: Array<any>) {
...
function displaySnapshot(snapshot: Snapshot) {
  ...
  detectAdjacentBadCids(snapshot.data.ops);
  detectInconsistentCids(snapshot.data.ops);
}
