...
export class AuthService {
  ...
  private async handleAuth(authResult: auth0.Auth0DecodedHash): Promise<boolean> {
    ...
    const newUser = prevUserId != null && prevUserId !== this.currentUserId;
    await Promise.all([
      this.orbitService.init(this.accessToken, newUser),
      this.realtimeService.init(this.accessToken, newUser)
    ]);
