...
describe('TextChooserDialogComponent', () => {
  ...
  it('indicates when not all segments of the end verse were selected', fakeAsync(async () => {
    const env = new TestEnvironment({ start: 3, end: TestEnvironment.verseLen(4) }, 'verse_1_3', 'verse_1_4');
    env.fireSelectionChange();
  ...
  it('indicates when not all segments of the start verse were selected', fakeAsync(async () => {
    const env = new TestEnvironment({ start: 0, end: TestEnvironment.verseLen(5) }, 'verse_1_4/p_1', 'verse_1_5');
    env.fireSelectionChange();
  ...
  it('indicates when the segments were only partially selected', fakeAsync(async () => {
    const env = new TestEnvironment({ start: 6, end: TestEnvironment.verseLen(5) - 2 }, 'verse_1_4', 'verse_1_5');
    env.fireSelectionChange();
  ...
  it('shows the correct verse range when first or last segment has only white space selected', fakeAsync(async () => {
    const env = new TestEnvironment({ start: TestEnvironment.verseLen(7) - 1, end: 1 }, 'verse_1_7', 'verse_1_9');
    env.fireSelectionChange();
  ...
  it("doesn't allow selecting whitespace", fakeAsync(() => {
    const env = new TestEnvironment({ start: TestEnvironment.verseLen(7) - 1, end: 1 }, 'verse_1_7', 'verse_1_8');
    env.fireSelectionChange();
    expect(env.selectedText).toEqual('');
    env.closeDialog();
  }));
  ...
  it('can handle selection of only whitespace', fakeAsync(() => {
    const env = new TestEnvironment(
      { start: TestEnvironment.verseLen(7) - 1, end: TestEnvironment.verseLen(7) },
      'verse_1_7',
      'verse_1_7'
    );
    expect(() => env.fireSelectionChange()).not.toThrow();
...
class TestEnvironment {
  ...
  static verseLen(verseNumber: number): number {
