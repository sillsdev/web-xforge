...
describe('CheckingComponent', () => {
  ...
  describe('Questions', () => {
    ...
    xit('allows admin to archive a question. PENDING: functionality is working, tests are not', fakeAsync(async () => {
      const env = new TestEnvironment(ADMIN_USER);
      env.selectQuestion(1);
    ...
    xit(
      'should move highlight and question icon when question is edited to move verses. ' +
        'PENDING: functionality is working, tests are not',
      fakeAsync(async () => {
        const env = new TestEnvironment(ADMIN_USER);
        env.selectQuestion(1);
        env.waitForSliderUpdate();
