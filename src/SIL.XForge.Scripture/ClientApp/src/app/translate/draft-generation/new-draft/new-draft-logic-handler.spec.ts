...
describe('NewDraftLogicHandler', () => {
  ...
  describe('initialization', () => {
    ...
    it('does not abort when a non-config field changes but the source set is unchanged', async () => {
      ...
      // Same project ref, but other fields (name, sync status, content) changed (e.g. after a sync). Source set is
      // unchanged, so this must not abort (content/sync changes are handled separately).
      sources$.next({
        trainingSources: [],
        trainingTargets: [],
  ...
  describe('training book selection', () => {
    ...
    it('limits a restored source selection to the restored target training books', async () => {
      // The saved source entry has books the saved target entry lacks. The UI normally prevents this state, but if
      // saved data carries it, the extra source books could never be seen or removed in the UI (the picker offers
      // only source books that are selected in the target), so they must not silently ride into the build.
      const testState = {
        ...teamStartingToTranslateGenesis,
        previouslySelectedTrainingScriptureRanges: [
          { projectId: 'training-source-1-id', scriptureRange: 'MAT;MRK;LUK;JHN' },
      ...
      expect(env.selectedTrainingSourceBooks).toEqual({ 'training-source-1-id': ['MAT', 'LUK'] });
    });
    ...
    it('auto-pairs every training source when the training sources were replaced since the last draft', async () => {
      ...
      // no saved entry. The target selection is still restored, the new source is paired with it, and the stale
      // saved entry does not linger in the selection.
      const testState = {
        ...teamStartingToTranslateGenesis,
        previouslySelectedTrainingScriptureRanges: [
  ...
  describe('reload (after in-place sync)', () => {
    ...
    it('re-reads progress for every involved project', async () => {
      // Which projects get an actual network refetch is ProgressService's concern: it refetches exactly the
      // projects that have synced since their cache entries were fetched (see its sync-token tests).
      const env = new TestEnvironment(teamStartingToTranslateGenesis);
      await env.waitForInit();
      ...
      verify(mockedDraftProgressService.getChaptersWithContent('testProjectId')).once();
      verify(mockedDraftProgressService.getChaptersWithContent('draft-source-1-id')).once();
      expect().nothing();
    });
...
class TestEnvironment {
  ...
  constructor(state: TestState, sources$?: BehaviorSubject<DraftSourcesAsArrays>) {
    ...
    if (state.targetProjectBooksChaptersAfterReload != null) {
      ...
      when(mockedDraftProgressService.getChaptersWithContent(projectId))
      ...
      when(mockedDraftProgressService.getChaptersWithContent(projectId)).thenResolve(
        new VerboseScriptureRange(state.targetProjectBooksChapters)
      );
    }
    when(mockedDraftProgressService.getPresentChapters(projectId)).thenResolve(
      new VerboseScriptureRange(state.targetProjectBooksChapters)
    );
    when(mockedDraftProgressService.getChaptersWithContent('draft-source-1-id')).thenResolve(
