...
export class EditorComponent extends DataLoadingComponent implements OnDestroy, AfterViewInit {
  ...
  @ViewChild('lineByLineInput') lineByLineInput?: ElementRef<HTMLInputElement>;
  ...
  get currentSourceText(): string {
    return this.source?.segmentText ?? '';
  }

  get currentTargetText(): string {
    return this.target?.segmentText ?? '';
  }
  ...
  get currentVerseRefForDisplay(): string {
    const segmentRef = this.target?.segment?.ref;
    if (segmentRef == null || this.bookNum == null) return '';

    const verseRef = getVerseRefFromSegmentRef(this.bookNum, segmentRef);
