...
import { RetryingRequest, TestingRetryingRequestService } from './retrying-request.service';
...
describe('RetryingRequest', () => {
  it('makes requests', async () => {
    ...
    expect(await request.complete).toBe('response');
  ...
  it('handles non-network errors', fakeAsync(() => {
    ...
    request.complete.catch(error => (rejectedWithError = error));
