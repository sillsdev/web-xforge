...
type ReportStyleUser = User & { id: string };
...
export class ExceptionHandlingService implements ErrorHandler {
   ...
   * null.
   */
  private async getUserForReporting(): Promise<ReportStyleUser | undefined> {
    if (!this.currentUser) {
      try {
    ...
    return user as ReportStyleUser;
  }

  private sendReport(error: any, user: ReportStyleUser, eventId: string) {
