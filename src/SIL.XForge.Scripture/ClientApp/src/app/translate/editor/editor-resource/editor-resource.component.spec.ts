...
describe('EditorResourceComponent', () => {
  ...
  it('should not init when projectId, bookNum, or chapter is undefined', () => {
    ...
    verify(mockSFProjectService.subscribeProfile(anything(), anything())).never();
  ...
  it('should init when projectId, bookNum, and chapter are defined', fakeAsync(() => {
    ...
    component.bookNum = 1;
    component.chapter = 1;
    when(mockSFProjectService.subscribeProfile(projectId, anything())).thenReturn(Promise.resolve(projectDoc));
    component['initProjectDetails']();
    component.resourceText.editorCreated.next();
    tick();
    verify(mockSFProjectService.subscribeProfile(projectId, anything())).once();
  ...
  it('can determine if a resource should display right-to-left', fakeAsync(() => {
    ...
    when(mockSFProjectService.subscribeProfile(projectId, anything())).thenReturn(Promise.resolve(rtlProjectDoc));
    component['initProjectDetails']();
    component.resourceText.editorCreated.next();
    tick();
  ...
  it('project copyright banner and copyright notice should init when they exist', fakeAsync(() => {
    ...
    when(mockSFProjectService.subscribeProfile(projectId, anything())).thenReturn(Promise.resolve(projectNoticeDoc));
    component['initProjectDetails']();
    component.resourceText.editorCreated.next();
    tick();
