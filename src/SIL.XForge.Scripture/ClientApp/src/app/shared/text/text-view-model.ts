...
export class TextViewModel {
  ...
  updateUsfmDescription(): void {
    ...
    const precedingParaIndex = Math.max(...styleOpIndexes.filter(i => i < highlightedSegmentIndex));
    const segmentIndex = delta.ops.findIndex((op, i) => i > precedingParaIndex && op.attributes?.segment != null);

    for (const [i, op] of delta.ops.entries()) {
      const len = typeof op.insert === 'string' ? op.insert.length : 1;
      clearDelta.retain(len, segmentIndex === i ? { 'style-description': description } : {});
    }
    editor.updateContents(clearDelta, 'silent');
  }
