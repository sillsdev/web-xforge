...
describe('BookMultiSelectComponent', () => {
  ...
  it('fetches progress only once for a project, no matter how many times the inputs change reference', async () => {
    // A consumer binding availableBooks/selectedBooks to getters hands new array instances on every change-detection
    // pass. The project hasn't changed, so progress must be fetched exactly once across all of them — re-fetching
    // (awaiting) on each pass is what spins into the infinite change-detection loop / freeze.
    for (let i = 0; i < 5; i++) {
      component.availableBooks = [...mockBooks];
      component.selectedBooks = [...mockSelectedBooks];
      await component.ngOnChanges();
