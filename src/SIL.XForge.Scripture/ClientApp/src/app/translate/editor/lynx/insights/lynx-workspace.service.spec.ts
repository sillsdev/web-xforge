...
describe('LynxWorkspaceService', () => {
  ...
  /** Long enough for both of the service's debounces (diagnostics settling, then insight emission) to elapse. */
  ...
  describe('2D Map Structure - Insights by URI and Source', () => {
    ...
    it('should clear insights of a source that no longer reports diagnostics', fakeAsync(() => {
      ...
      // Source-a's diagnostic is resolved (as when its insight action is applied), so the next event carries only
      // source-b's diagnostic
      env.diagnosticsChangedTestSubject$.next({
        uri: textDocId.toString(),
