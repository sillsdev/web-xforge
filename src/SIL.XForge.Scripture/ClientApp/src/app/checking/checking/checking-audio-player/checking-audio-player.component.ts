...
export class CheckingAudioPlayerComponent implements OnDestroy {
  ...
  @Input() set source(source: string) {
    if (source && source !== '') {
      ...
      // By making it skip to the end the duration becomes available. To do this we have to skip to some point that
      // we assume is past the end. This number should be large, but numers as small as 1e16 have been observed to cause
      // audio playback to skip to the end of the audio when the user presses play in Chromium.
      // Normal audio files will know the duration once metadata has loaded.
