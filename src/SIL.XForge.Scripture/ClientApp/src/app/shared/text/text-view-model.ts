...
export class TextViewModel implements OnDestroy, LynxTextModelConverter {
  ...
  private fixSegment(
    ...
    if (segment.length - segment.notesCount === 0 && isOnline) {
      ...
      // Remove the blank from the embed positions
      [...this._embeddedElements].forEach(([key, value]) => {
        if (value.position === position) {
          this._embeddedElements.delete(key);
        }
      });
    ...
    for (let i = 0; i < modelDelta.ops.length; i++) {
      ...
      if (op.insert === '\n' || op.attributes?.para != null || op.attributes?.book != null) {
        ...
        if ((op.attributes as any)?.segment == null) {
          console.log(`Missing Segment: ${JSON.stringify(op)}`);
        } else {
