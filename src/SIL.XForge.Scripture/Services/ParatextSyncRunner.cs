    ...
    public class ParatextSyncRunner : IParatextSyncRunner
    {
        ...
        public async Task RunAsync(string projectId, string userId, bool trainEngine, CancellationToken token)
        {
            ...
            try
            {
                ...
                Log($"RunAsync: Will use target project Paratext ID '{targetParatextId}' and source project Paratext ID '{sourceParatextId}'.");
                ...
                {
                    Log($"RunAsync: A local PT repo backup exists.");
                }
                ...
                if (lastSharedVersion == _projectDoc.Data.Sync.SyncedToRepositoryVersion)
                {
                    ...
                    isDataInSync = true;
                    Log($"RunAsync: The local PT repo's last shared revision matches the SF DB project Sync.SyncedToRepositoryVersion of '{lastSharedVersion}'. Considering that data is in sync.");
                }
                ...
                if (isDataInSync)
                {
                    Log($"RunAsync: Updating texts and questions,answers,comments in local PT repo working dir, since data in sync.");
                }
                else
                {
                    Log($"RunAsync: Not writing texts and questions,answers,comments to local PT repo working dir, since data not in sync.");
                }
                ...
                {
                    Log($"Updating user access list to resource.");
                ...
                Log($"RunAsync: Updating SF DB texts,questions,answers,comments.");
                ...
                {
                    Log($"RunAsync: Starting Machine build.");
        ...
        {
            Log($"Starting.", projectSFId, userId);
        ...
        private async Task CompleteSync(bool successful, bool canRollbackParatext, CancellationToken token)
        {
            ...
            {
                Log($"CompleteSync: Sync was not successful. Rolling back local PT repo.");
                bool rollbackOutcome = _paratextService.RestoreRepository(_userSecret, _projectDoc.Data.ParatextId);
                Log($"CompleteSync: This local PT repo roll back was successful: {rollbackOutcome}");
            }
            else if (!successful)
            {
                Log($"CompleteSync: Sync was not successful. But we don't think we can roll back the local PT repo, so not doing that.");
            }
            ...
            if (successful)
            {
                Log($"CompleteSync: Committing transaction to SF DB.");
                ...
                {
                    Log($"CompleteSync: Backing up local PT repo.");
                ...
                {
                    Log($"CompleteSync: Not backing up a local resource.");
                }
            }
            else
            {
                Log($"CompleteSync: Not committing transaction to SF DB after failed synchronization attempt. Rolling back SF DB transaction.");
