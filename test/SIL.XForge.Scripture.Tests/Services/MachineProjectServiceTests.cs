...
public class MachineProjectServiceTests
{
    ...
    public async Task BuildProjectAsync_DoesNotBuildServalIfNoTextChanges()
    {
        // Set up test environment
        var env = new TestEnvironment(
            new TestEnvironmentOptions { SourceTextCorpusHasData = true, TargetTextCorpusHasData = true }
        );
        await env.SetSourceAndTargetChecksumToMatchTextCorpusAsync(Project02);
    ...
    {
        // Set up test environment
        var env = new TestEnvironment(new TestEnvironmentOptions { SourceTextCorpusHasData = true });
        await env.CreateEmptyServalDataAsync(Project01);
    ...
    public async Task SyncProjectCorporaAsync_DoesNotUpdateIfNoText()
    {
        // Set up test environment
        var env = new TestEnvironment();
        await env.ClearServalFilesAsync(Project02);
    ...
    public async Task SyncProjectCorporaAsync_UpdatesCorpusIfTextExists()
    {
        ...
        // Setup just one file that will be updated
    ...
    public async Task SyncProjectCorporaAsync_AddsAndDeletesSourceAndTargetFiles()
    {
        ...
        // Specify the source and target files that will be deleted
        await env.ProjectSecrets.UpdateAsync(
            Project02,
            u =>
                u.Add(
    ...
    public async Task SyncProjectCorporaAsync_DoesNotCrashWhenDeletingDeletedFiles()
    {
        ...
        // Specify the one file we will delete
        await env.ProjectSecrets.UpdateAsync(
            Project02,
            u =>
                u.Add(
    ...
    private class TestEnvironmentOptions
    {
        ...
        public bool SourceTextCorpusHasData { get; init; }
        public bool TargetTextCorpusHasData { get; init; }
    }
    ...
    private class TestEnvironment
    {
        public TestEnvironment(TestEnvironmentOptions? options = null)
        {
            ...
            if (options.SourceTextCorpusHasData && options.TargetTextCorpusHasData)
            {
                textCorpusFactory
                    .CreateAsync(Arg.Any<IEnumerable<string>>(), Arg.Any<TextCorpusType>())
                    .Returns(MockTextCorpus);
            }
            else if (options.SourceTextCorpusHasData)
            {
                textCorpusFactory
                    .CreateAsync(Arg.Any<IEnumerable<string>>(), TextCorpusType.Source)
                    .Returns(MockTextCorpus);
            }
            else if (options.TargetTextCorpusHasData)
            {
                textCorpusFactory
        ...
        public async Task SetSourceAndTargetChecksumToMatchTextCorpusAsync(string projectId) =>
        ...
        public async Task ClearServalFilesAsync(string projectId) =>
            await ProjectSecrets.UpdateAsync(
                projectId,
                u =>
                {
        ...
        public async Task CreateEmptyServalDataAsync(string projectId) =>
            await ProjectSecrets.UpdateAsync(
                projectId,
